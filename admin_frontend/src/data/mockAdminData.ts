...
import { SAT_CLOSED_AFTER } from "@/lib/constants";
...
// Horario operativo 09:00–18:00 únicamente
...
// Sábado: cerrado en horas > SAT_CLOSED_AFTER
const RAW_GRID: Record<string, number[]> = {
    ...
    S: [20, 30, 40, 50,  0,  0,  0,  0,  0,  0], // índices 4..9 = horas 13..18 → Cerrado
};

function toLevel(occ: number): "Bajo" | "Medio" | "Alto" | "Crítico" | "Cerrado" {
    if (occ === 0) return "Cerrado";
...
function isSatClosed(day: string, hourStr: string): boolean {
    return day === "S" && parseInt(hourStr, 10) > SAT_CLOSED_AFTER;
}
...
export const MOCK_HEATMAP: WeeklyHeatmapResponse = {
    ...
    items: DAYS.flatMap((day) =>
        HOURS.map((hour, i) => {
            const closed = isSatClosed(day, hour);
            const occ    = closed ? 0 : RAW_GRID[day][i];
            return { day, hour, occupancy: occ, level: toLevel(occ) };
        })
    ),
};
...
export const MOCK_RECOMMENDATIONS: RecommendationResponse = {
    ...
    items: [
        ...
        { rank: 4, start_time: "17:00", end_time: "18:00", score: 75, level: "Medio", reason: "Franja vespertina con demanda moderada." },
...
export const MOCK_DATA_SOURCES: DataSourcesResponse = {
    sources: [
        { name: "log_gym.xlsx",                     layer: "raw",       description: "Registros históricos del gimnasio UTEC",              required: true,  available: true },
